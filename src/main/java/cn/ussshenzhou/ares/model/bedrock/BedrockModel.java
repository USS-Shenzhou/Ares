...
import cn.ussshenzhou.ares.model.bedrock.data.BedrockModelPOJO;
import cn.ussshenzhou.ares.model.bedrock.data.Bones;
import cn.ussshenzhou.ares.model.bedrock.data.Cubes;
import cn.ussshenzhou.ares.model.bedrock.data.Description;
...
public class BedrockModel<T extends EntityRenderState> {
    ...
    protected void loadNewModel(BedrockModelPOJO pojo) {
        ...
        for (BonesItem bones : pojo.getGeometryModel().getBones()) {
            ...
            parts.put(bones.getName(), new ModelRendererWrapper(new BedrockPart()));
        }
        ...
        for (BonesItem bones : pojo.getGeometryModel().getBones()) {
            ...
            // 塞进 HashMap 里面的模型对象
            BedrockPart model = parts.get(name).getModelRenderer();
            ...
            // Null 检查，进行父骨骼绑定
            if (parent != null) {
                parts.get(parent).getModelRenderer().addChild(model);
            } else {
                // 没有父骨骼的模型才进行渲染
        ...
        if (animations != null) {
            ...
            if (entityIn instanceof Mob mob) {
                ...
                if (maid != null) {
                    ...
                    // 硬编码动画
                    HardcodedAnimationManger.playMaidAnimation(maid, parts, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch);
                }
                return;
            }
        ...
        try {
            for (Object animation : animations) {
                if (animation instanceof IAnimation) {
                    ((IAnimation<Mob>) animation).setRotationAngles(limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, 0, entityIn.asEntity(), parts);
                } else {
                    entityMaidWrapper.setData(entityIn, attackTime, riding);
                    invocable.invokeMethod(animation, "animation", entityMaidWrapper, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, 0.0625f, parts);
    ...
    private void setupChairAnim(EntityChair entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, Invocable invocable) {
        try {
            for (Object animation : animations) {
                if (animation instanceof IAnimation) {
                    ((IAnimation<EntityChair>) animation).setRotationAngles(limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, 0, entityIn, parts);
                } else {
                    entityChairWrapper.setData(entityIn);
                    invocable.invokeMethod(animation, "animation", entityChairWrapper, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, 0.0625f, parts);
    ...
    public boolean hasBackpackPositioningModel() {
        return parts.get("backpackPositioningBone") != null;
    }

    public BedrockPart getBackpackPositioningModel() {
        return parts.get("backpackPositioningBone").getModelRenderer();
    }

    @Nullable
    private BedrockPart getArm(HumanoidArm sideIn) {
        return sideIn == HumanoidArm.LEFT ? parts.get("armLeft").getModelRenderer() : parts.get("armRight").getModelRenderer();
    }

    public boolean hasHead() {
        return parts.containsKey("head");
    }

    public BedrockPart getHead() {
        return parts.get("head").getModelRenderer();
    }

    public boolean hasLeftArm() {
        return parts.containsKey("armLeft");
    }

    public boolean hasRightArm() {
        return parts.containsKey("armRight");
    }

    public boolean hasArmPositioningModel(HumanoidArm side) {
        ModelRendererWrapper arm = (side == HumanoidArm.LEFT ? parts.get("armLeftPositioningBone") : parts.get("armRightPositioningBone"));
        return arm != null;
    }

    public void translateToPositioningHand(HumanoidArm sideIn, PoseStack poseStack) {
        BedrockPart arm = (sideIn == HumanoidArm.LEFT ? parts.get("armLeftPositioningBone").getModelRenderer() : parts.get("armRightPositioningBone").getModelRenderer());
        if (arm != null) {
            arm.translateAndRotate(poseStack);
        }
    }
    ...
    public boolean hasWaistPositioningModel(HumanoidArm side) {
        ModelRendererWrapper waist = (side == HumanoidArm.LEFT ? parts.get("waistLeftPositioningBone") : parts.get("waistRightPositioningBone"));
        return waist != null;
    }

    public void translateToPositioningWaist(HumanoidArm sideIn, PoseStack poseStack) {
        BedrockPart waist = (sideIn == HumanoidArm.LEFT ? parts.get("waistLeftPositioningBone").getModelRenderer() : parts.get("waistRightPositioningBone").getModelRenderer());
        if (waist != null) {
    ...
    private float convertPivot(BonesItem parent, Cubes cube, int index) {
     ...
     * @param index 是 xyz 的哪一个，x 是 0，y 是 1，z 是 2
     */
    private float convertOrigin(BonesItem bone, Cubes cube, int index) {
        if (index == 1) {
    ...
    private float convertOrigin(Cubes cube, int index) {
        assert cube.getPivot() != null;
        if (index == 1) {
    ...
    public HashMap<String, BedrockPart> getParts() {
        return parts;
    }
}