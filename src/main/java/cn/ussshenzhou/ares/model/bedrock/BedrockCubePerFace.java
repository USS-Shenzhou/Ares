...
import cn.ussshenzhou.ares.model.bedrock.data.Face;
import cn.ussshenzhou.ares.model.bedrock.data.FaceContainer;
 ...
 */
public class BedrockCubePerFace implements IBedRockRenderable {
    ...
    public BedrockCubePerFace(float x, float y, float z, float width, float height, float depth, float delta, float texWidth, float texHeight, FaceContainer faces) {
        this.minX = x;
        this.minY = y;
        this.minZ = z;
        this.maxX = x + width;
    ...
    private BedrockPolygon getTexturedQuad(BedrockVertex[] positionsIn, float texWidth, float texHeight, Direction direction, FaceContainer faces) {
        Face face = faces.getFace(direction);
